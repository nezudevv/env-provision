...
8. Installs global npm packages: @vue/language-server (required for Vue3 TypeScript support in Neovim)
9. Installs Nerd Fonts (with error handling for existing fonts)
10. Checks if Powerlevel10k configuration exists (~/.p10k.zsh) and logs warning if missing
...
### 10. Neovim Vue3 Support
The Neovim config supports Vue3 files with full TypeScript integration. This requires `@vue/language-server` to be installed globally via npm (automatically installed by `00-install-dependencies`). The LSP config in `.config/nvim/lua/nezudevv/plugins/nvim-lspconfig.lua` sets up both `ts_ls` (TypeScript Language Server) and `vue_ls` (Volar) to work together. The `ts_ls` loads `@vue/typescript-plugin` from the global npm packages to enable TypeScript features in Vue single-file components.
...
1. For Homebrew packages: Add to `brew_packages` array in `runs/00-install-dependencies`
2. For npm packages: Add to `npm_packages` array in `runs/00-install-dependencies`
3. If it's a font, add to `fonts` array
4. If it requires special setup, add after the appropriate installation section
